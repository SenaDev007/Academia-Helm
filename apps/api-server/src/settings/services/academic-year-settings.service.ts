  ...
  async getYearStats(tenantId: string, id: string) {
    ...
    return {
      ...
      stats: {
        ...
        canPromote: year.isActive && !year.isClosed,
      },
    };
  }

  /**
   * Clôture l'année active courante ET active la suivante en une seule
   * transaction. Crée aussi les enrollments PROMOTION pour chaque élève
   * actif de l'année clôturée — c'est l'opération "Passer à l'année suivante".
   *
   * Différents de `close()` qui refuse de clôturer l'année active :
   *   - `close()`    = clôture seule (l'année doit être inactive au préalable)
   *   - `promote()`  = clôture + activation suivante + promotion élèves
   *
   * Délègue la logique transactionnelle à `AcademicYearsPrismaService.closeAndPromoteYear()`
   * qui gère tout en une transaction atomique.
   *
   * @param tenantId Le tenant
   * @param id       L'ID de l'année à clôturer+promouvoir (doit être l'active)
   * @param userId   L'utilisateur qui déclenche l'action (ou 'SYSTEM_CRON')
   */
  async promote(
    tenantId: string,
    id: string,
    ...
    // On utilise directement PrismaService pour la transaction complète.
    // Réimplémentation locale pour ne pas dépendre du module academic-years
    // (qui contient aussi du code legacy cassé) — on garde la cohérence
    // avec le calendrier type Bénin via les méthodes privées de calcul.
    const year = await this.getById(tenantId, id);
    if (year.isClosed) {
      throw new BadRequestException(`L'année ${year.name} est déjà clôturée.`);
    }
    if (!year.isActive) {
      throw new BadRequestException(
        'Seule l\'année active peut être promue. Activez-la d\'abord, ou utilisez "Clôturer" pour une année inactive.',
      );
    }

    // Calculer les dates de l'année suivante à partir du nom "YYYY-YYYY"
    const startYear = this.parseStartYearFromName(year.name);
    if (startYear === null) {
      throw new BadRequestException(
        `Nom d'année invalide (${year.name}). Format attendu : YYYY-YYYY (ex : 2025-2026).`,
      );
    }
    ...
    // Transaction atomique : clôture + upsert closure + création/activation suivante + promotions
    const nextYear = await this.prisma.$transaction(async (tx) => {
      ...
      // 2. Créer ou mettre à jour l'entrée AcademicYearClosure
      await tx.academicYearClosure.upsert({
        ...
        update: { closedAt, closedBy: userId, isLocked: true },
        create: {
          tenantId,
          academicYearId: year.id,
          closedAt,
      ...
      // 3. Upsert l'année suivante (la créer si elle n'existe pas, sinon juste l'activer)
      const existing = await tx.academicYear.findFirst({
        where: { tenantId, name: nextName },
      });
      let nextYearRecord: any;
      if (existing) {
        nextYearRecord = await tx.academicYear.update({
          where: { id: existing.id },
          data: {
            ...prismaUpdateDefaults(),
            isActive: true,
            preEntryDate: nextDates.preEntryDate,
            officialStartDate: nextDates.officialStartDate,
            startDate: nextDates.startDate,
            endDate: nextDates.endDate,
          },
        });
      } else {
        nextYearRecord = await tx.academicYear.create({
          data: {
            ...prismaCreateDefaults(),
            tenantId,
            name: nextName,
      ...
      // 4. Créer les 3 trimestres par défaut pour la nouvelle année si elle n'en a pas
      const existingPeriods = await tx.academicPeriod.findMany({
        where: { tenantId, academicYearId: nextYearRecord.id },
      });
      if (existingPeriods.length === 0) {
        await this.academicPeriodSettingsService.createDefaultTrimestersForYear(
          tenantId,
          nextYearRecord.id,
          nextDates.startDate,
          nextDates.endDate,
      ...
      // 5. Promotion : créer un enrollment PROMOTION pour chaque élève actif
      ...
      for (const enrollment of activeEnrollments) {
        await tx.studentEnrollment.create({
          data: {
            tenantId,
            academicYearId: nextYearRecord.id,
    ...
    await this.historyService.logSettingChange(
      ...
      {
        promoted: {
          old: { yearName: year.name, yearId: year.id },
          new: { yearName: nextYear.name, yearId: nextYear.id, promotedEnrollments: true },
        },
      },
      userId,
    );
    ...
    this.logger.log(
      `✓ Année ${year.name} clôturée et promue → ${nextYear.name} (tenant ${tenantId}, user ${userId})`,
    );

    return {
      previousYearId: year.id,
      previousYearLabel: year.name,
      nextYearId: nextYear.id,
      nextYearLabel: nextYear.name,
      closedAt,
    };
  }
}
