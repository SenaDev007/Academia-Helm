/**
 * HRSaraAssistant Component
 * 
 * Assistant IA (SARA) pour la gestion des Ressources Humaines.
 * Aide au recrutement, évaluation de performance et conseils stratégiques RH.
 */
...
export default function HRSaraAssistant() {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: "Analyse des compétences en cours... J'ai identifié un besoin d'accompagnement sur les outils numériques pour 12 professeurs. Voulez-vous que je rédige une proposition de plan de formation ?" 
      }]);
    }, 1200);
  };
  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 w-16 h-16 bg-blue-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all z-50 group border border-white/20"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-28 right-8 w-[400px] h-[700px] bg-slate-900 text-white rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.5)] border border-slate-800 flex flex-col z-50 overflow-hidden"
          >
            {/* Premium Header */}
            <div className="p-8 bg-gradient-to-br from-blue-700 to-indigo-900 relative overflow-hidden">
            ...
            <div className="px-8 py-4 bg-slate-900/50 border-b border-slate-800 flex gap-2 overflow-x-auto no-scrollbar">
              {[
                { icon: UserCheck, label: 'Performance', color: 'text-emerald-400' },
                { icon: FileSearch, label: 'Audit Contrats', color: 'text-blue-400' },
                { icon: Target, label: 'Formation', color: 'text-amber-400' },
              ].map((item, i) => (
                <button key={i} className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-full border border-slate-700 hover:bg-slate-700 transition-all whitespace-nowrap">
            ...
            <div className="p-6 bg-slate-900 border-t border-slate-800">
               <div className="flex items-center gap-3 bg-slate-950 rounded-2xl p-3 border border-slate-800">
                  <input 
                    ...
                    placeholder="Posez une question stratégique RH..."
