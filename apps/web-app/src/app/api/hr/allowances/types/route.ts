...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/hr/allowances/types${queryString ? `?${queryString}` : ''}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/hr/allowances/types`, {
      method: 'POST',
      headers: {
