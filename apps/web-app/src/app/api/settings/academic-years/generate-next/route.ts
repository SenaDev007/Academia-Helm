/**
 * API PROXY - Générer la prochaine année scolaire
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/settings/academic-years/generate-next`, {
      method: 'POST',
      headers,
    });
    ...
    return NextResponse.json({ error: 'Failed to generate next academic year' }, { status: 500 });
  }
}
