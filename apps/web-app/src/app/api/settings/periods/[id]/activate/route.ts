/**
 * API PROXY - Activer une période académique
 */
...
export async function POST(
  ...
  try {
    ...
    const url = new URL(`${API_BASE_URL}/settings/periods/${id}/activate`);
    const fromQuery = request.nextUrl?.searchParams?.get('tenant_id');
    ...
    return NextResponse.json({ error: 'Failed to activate period' }, { status: 500 });
  }
}
