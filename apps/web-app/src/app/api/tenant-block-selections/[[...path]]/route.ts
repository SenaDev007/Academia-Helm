/**
 * ============================================================================
 * Proxy API → NestJS /tenant-block-selections/*
 * ============================================================================
 *
 * Persistance des sélections de composants CMS (navbar, hero, footer, etc.)
 *
 * Endpoints proxyés :
 *   GET    /api/tenant-block-selections              → toutes les sélections (auth)
 *   GET    /api/tenant-block-selections/:category     → une catégorie (auth)
 *   PUT    /api/tenant-block-selections/:category     → upsert (auth)
 *   DELETE /api/tenant-block-selections/:category     → supprime (auth)
 *   GET    /api/tenant-block-selections/public/:slug  → public (no auth)
 * ============================================================================
 */
...
function buildBackendUrl(pathSegments: string[]): string {
  ...
  return nestControllerUrl(`tenant-block-selections${path ? `/${path}` : ''}`);
}
...
async function forward(
  ...
  try {
    ...
    console.error('[api/tenant-block-selections]', e);
