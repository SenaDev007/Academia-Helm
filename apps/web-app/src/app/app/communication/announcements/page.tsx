      ...
      <div className="p-6 space-y-8">
        ...
        <div className="bg-blue-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-blue-200">
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            <div className="flex gap-4 mt-8">
              <button className="bg-white text-blue-600 px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-50 transition-all shadow-lg">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-100 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 transition-all"
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          {announcements.map((ann) => (
            <div key={ann.id} className="group bg-white rounded-3xl p-6 border border-slate-100 hover:shadow-xl hover:shadow-blue-100/30 transition-all flex flex-col md:flex-row md:items-center gap-6">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${ann.priority === 'CRITICAL' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600'} group-hover:scale-105 transition-transform`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <h4 className="font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors">{ann.title}</h4>
                  {getPriorityBadge(ann.priority)}
                </div>
                <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
                  <span className="flex items-center gap-1.5"><Users size={12} className="text-blue-400" /> {ann.audience}</span>
              ...
              <div className="flex items-center gap-6 pl-6 border-l border-slate-50">
                <div className="text-center">
                  <p className="text-xl font-black text-slate-900 flex items-center gap-1">
                    <Eye size={16} className="text-blue-500" /> {ann.views}
                  </p>
        ...
        {/* Empty State Action */}
        <button className="w-full py-12 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-slate-300 font-black uppercase tracking-widest text-sm hover:border-blue-300 hover:text-blue-400 hover:bg-blue-50/20 transition-all flex flex-col items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-blue-100">
            <Plus size={32} />
          </div>
