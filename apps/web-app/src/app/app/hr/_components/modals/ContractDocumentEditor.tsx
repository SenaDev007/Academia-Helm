...
const RichTextContent = memo(function RichTextContent({ initialContent, onInput, onBlur }: RichTextContentProps) {
...
export function ContractDocumentEditor({ isOpen, onClose, onSuccess, contract }: ContractDocumentEditorProps) {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      if (data?.templateVars) {
        ...
        setArticles([
          { title: "Article 1 — Objet du contrat", content: rv(`Le présent contrat a pour objet l'engagement du Salarié en qualité de <strong>{{staffPosition}}</strong> au sein de l'établissement <strong>{{schoolName}}</strong>. Le Salarié exercera ses fonctions sous l'autorité de la Direction de l'établissement scolaire.`) },
          { title: "Article 2 — Date d'effet", content: rv(`Le présent contrat prend effet à compter du <strong>{{startDate}}</strong>.`), },
          { title: "Article 3 — Type de contrat", content: rv(isCDI ? `Le présent contrat est un <strong>Contrat à Durée Indéterminée (CDI)</strong>.` : isConsultant ? `Le présent contrat est un <strong>Contrat de Consultation</strong> pour la période du <strong>{{startDate}}</strong> au <strong>{{endDate}}</strong>.` : `Le présent contrat est conclu pour la période du <strong>{{startDate}}</strong> au <strong>{{endDate}}</strong>.`), },
          { title: "Article 4 — Période d'essai", content: rv(`Le présent contrat est assorti d'une période d'essai de <strong>{{probationDuration}}</strong>. Durant cette période, chacune des parties pourra mettre fin au contrat conformément aux dispositions légales en vigueur.`), },
          { title: "Article 5 — Missions et responsabilités", content: rv(`Le Salarié s'engage à accomplir notamment les missions suivantes : <strong>{{jobResponsibilities}}</strong>. Cette liste n'est pas exhaustive et peut être adaptée en fonction des besoins de l'établissement.`), },
          { title: "Article 6 — Lieu de travail", content: rv(`Le Salarié exercera principalement ses fonctions à : <strong>{{workLocation}}</strong>. Toute affectation dans un autre lieu de travail pourra être décidée par l'Employeur, après consultation du Salarié, dans l'intérêt du service.`), },
          { title: "Article 7 — Durée du travail", content: rv(`La durée hebdomadaire de travail est fixée à <strong>{{weeklyHours}} heures</strong>. Les horaires de travail sont : {{workSchedule}}. Toute modification des horaires sera portée à la connaissance du Salarié dans un délai raisonnable.`), },
          { title: "Article 8 — Rémunération", content: rv(isStage ? `Le stagiaire percevra une gratification mensuelle nette de <strong>{{baseSalary}} {{currency}}</strong>, payée par <strong>{{paymentMode}}</strong>.` : `En contrepartie de son travail, le Salarié percevra une rémunération composée comme suit :<br>- Salaire de base : <strong>{{baseSalary}} {{currency}}</strong><br>- Prime de fonction : <strong>{{functionBonus}} {{currency}}</strong><br>- Indemnité de transport : <strong>{{transportBonus}} {{currency}}</strong><br>- Autres avantages : <strong>{{otherBenefits}}</strong><br><br>Soit un salaire brut mensuel de <strong>{{grossSalary}} {{currency}}</strong>, versé mensuellement par <strong>{{paymentMode}}</strong>.`), },
          { title: "Article 9 — Congés", content: rv(`Le Salarié bénéficiera des congés conformément à la législation du travail en vigueur dans le pays ({{country}}). Les périodes de congé seront fixées d'un commun accord entre l'Employeur et le Salarié, dans le respect du fonctionnement de l'établissement scolaire et du calendrier académique.`), },
          ...
          { title: "Article 15 — Droit applicable", content: rv(`Le présent contrat est soumis au droit du travail applicable en <strong>{{country}}</strong>. Toute clause non prévue expressément sera régie par les dispositions légales et conventionnelles en vigueur.`), },
          { title: "Article 16 — Dispositions finales", content: rv(`Le présent contrat est établi en deux (2) exemplaires originaux, dont un remis à chaque partie. Les modifications apportées au présent contrat feront l'objet d'un avenant écrit. Fait à <strong>{{city}}</strong>, le <strong>{{signatureDate}}</strong>.`), },
        ]);
      }
      setHasChanges(false);
    } catch (err) {
  ...
  /**
   * Recalcule le salaire brut mensuel de l'Article 8 et met à jour le HTML.
   *
   * ⚠️ Cette fonction est appelée UNIQUEMENT au blur (quand l'utilisateur
   * quitte l'éditeur de l'Article 8), JAMAIS pendant la frappe. Sinon, le
   * RichTextContent (uncontrolled) re-set innerHTML à chaque modification
   * → le caret saute au début du contenu → experience catastrophique.
   *
   * Étapes :
   ...
   *
   * Si le parsing échoue (format inattendu), on ne modifie rien.
   */
  function recalculateGrossSalaryOnBlur(index: number) {
    setArticles((prev) => {
      const updated = [...prev];
      const article = updated[index];
      if (!article?.title?.includes('Rémunération')) return prev;
      ...
      try {
        ...
        // Si pas de ligne "salaire brut mensuel" → pas de modification
        if (currentTotal === -1) return prev;

        // Si le total est déjà correct → pas de modification
        ...
        updated[index] = { ...updated[index], content: updatedHtml };
        return updated;
      } catch {
        return prev;
      }
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="border border-slate-200 rounded-lg overflow-hidden focus-within:border-[#1A2BA6] focus-within:ring-2 focus-within:ring-[#1A2BA6]/10">
                    ...
                    <RichTextContent
                      ...
                      onBlur={() => recalculateGrossSalaryOnBlur(idx)}
                    />
                  </div>
                </div>
              </motion.div>
