      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="relative">
            ...
            <input required type="text" placeholder="Ex : Professeur de SVT, Comptable..." className={inputClass + ' pl-9'} value={employment.position} onChange={(e) => onUpdate('position', e.target.value)} />
          </div>
        </div>
        <div>
          <label className={labelClass}>D&eacute;partement</label>
          <input type="text" placeholder="Ex : P&eacute;dagogie, Administration..." className={inputClass} value={employment.department} onChange={(e) => onUpdate('department', e.target.value)} />
        </div>
      </div>
