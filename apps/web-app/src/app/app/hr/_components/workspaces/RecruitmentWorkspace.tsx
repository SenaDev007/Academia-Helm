        ...
        <AnimatePresence mode="wait">
                          ...
                          {job.status === 'PUBLIÉE' && (() => {
                            ...
                            return (
                              <div className="flex items-center gap-1">
                                {/* Facebook — logo officiel bleu */}
                                <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedText}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded transition hover:opacity-80" title="Partager sur Facebook">
                                ...
                                {/* X (Twitter) — logo officiel noir */}
                                <a href={`https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded transition hover:opacity-80" title="Partager sur X">
                                  <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="#000000"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                                </a>
                                {/* LinkedIn — logo officiel bleu */}
                                <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded transition hover:opacity-80" title="Partager sur LinkedIn">
                                ...
                                {/* WhatsApp — logo officiel vert */}
                                <a href={`https://wa.me/?text=${encodedText}%20${encodedUrl}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded transition hover:opacity-80" title="Partager sur WhatsApp">
                                ...
                                {/* Copier le lien */}
                                <button onClick={() => { navigator.clipboard.writeText(shareUrl); toast({ variant: 'success', title: 'Lien copié !' }); }} className="p-1 rounded text-slate-400 hover:text-slate-600 transition" title="Copier le lien">
          ...
          {activeTab === 'tests' && (
            <motion.div key="tests" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              {/* ─── Système unifié : Questionnaires + Programmation + Résultats ─── */}
