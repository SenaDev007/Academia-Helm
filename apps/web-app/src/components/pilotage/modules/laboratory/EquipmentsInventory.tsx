...
import { Loader2, Microscope, Search, Filter, MoreVertical, Wrench, Download, Plus } from 'lucide-react';
...
export default function EquipmentsInventory() {
  ...
  const { data: equipments, loading, error } = useModulesList<EquipmentItem>(
    'labs',
    'equipment',
    academicYear?.id,
  );
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-900 group-hover:text-blue-600 transition-colors">{eq?.name ?? `Équipement #${i + 1}`}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{eq?.id ?? `EQ-${i}`}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5">
