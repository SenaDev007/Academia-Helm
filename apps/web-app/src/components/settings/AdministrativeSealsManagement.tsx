...
// Modal de gestion des versions de cachet
function SealVersionModal({ seal, onClose, onSuccess }: { seal: AdministrativeSeal; onClose: () => void; onSuccess: () => void }) {
  const [submitting, setSubmitting] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Une erreur est survenue lors de la création de la version');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div className="p-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4">
            Nouvelle version - {seal.label}
          </h3>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Image du cachet (PNG/SVG transparent recommandé)
              </label>
              <input
                type="file"
                required
            ...
            <div className="flex items-center justify-end gap-3 pt-4">
              ...
              >
                {submitting ? 'Upload en cours...' : 'Créer la version'}
              </button>
            </div>
          </form>
        </div>
