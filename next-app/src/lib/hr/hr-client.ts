...
import { getClientAuthorizationHeader, getClientTenantId, tryRefreshAccessToken } from '@/lib/auth/client-access-token';
...
export async function hrFetch<T>(
  ...
  const res = await fetch(fetchUrl, {
    ...
    headers: {
      ...
      ...(getClientTenantId() ? { 'X-Tenant-ID': getClientTenantId()! } : {}),
      ...options?.headers,
    },
    credentials: 'include',
    cache: 'no-store',
